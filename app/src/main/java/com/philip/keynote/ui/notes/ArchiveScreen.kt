                    ...
                    Card(
                        ...
                        Row(
                            ...
                            Column(modifier = Modifier.weight(1f)) {
                                ...
                                Text(
                                    text = "Diarsipkan: ${dateFormat.format(Date(note.updatedAt))}",
                                    style = MaterialTheme.typography.bodySmall,
                                    color = contentColor.copy(alpha = 0.7f)
                                )
                            }
                            IconButton(onClick = { viewModel.restoreFromArchive(note) }) {
                                Icon(
                                    Icons.Default.Unarchive,
                                    contentDescription = "Kembalikan ke Catatan Utama",
                                    tint = contentColor
                                )
                            }
                        }
